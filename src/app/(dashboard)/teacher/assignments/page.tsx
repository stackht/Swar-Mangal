...
export default function TeacherAssignmentsPage() {
  const { assignments, students, refetch, isDemo , currentTeacherId } = useAcademyData();
  const [list, setList] = React.useState<Assignment[]>(assignments.filter((a) => a.teacher_id === currentTeacherId));
  ...
  const review = async (a: Assignment) => {
    ...
    setReviewing(null);
    setFeedback("");
    toast.success("Assignment reviewed â€” feedback sent");
  };
    ...
    <div>
      ...
      <section className="mb-8">
        ...
        <div className="space-y-3">
          {submitted.length === 0 && <p className="rounded-3xl border bg-card p-8 text-center text-sm text-muted-foreground">Nothing to review ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â all caught up!</p>}
          {submitted.map((a, i) => {
                  ...
                  <CardContent className="p-5">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">by {student?.full_name ?? "Student"} Ãƒâ€šÃ‚Â· submitted recently</p>
                      </div>
                      <Dialog open={reviewing?.id === a.id} onOpenChange={(o) => !o && setReviewing(null)}>
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>by {student?.full_name} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â expected {a.expected_minutes} min</DialogDescription>
                          </DialogHeader>
            ...
            return (
              <div key={a.id} className="flex items-center gap-4 rounded-2xl border bg-card p-4 shadow-card">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{student?.full_name} Ãƒâ€šÃ‚Â· due {new Date(a.due_date).toLocaleDateString()}</p>
                </div>
