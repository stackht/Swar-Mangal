...
import { teacherStatusTone } from "../_shared";

type AddTeacherArg = { teacherName: string; phone: string; primaryRole: string };
type AddTeacherRes = RpcEnvelope & { teacherId?: string };
...
export default function FounderTeachersPage() {
  ...
  const addTeacher = useMutationRpc<AddTeacherArg, AddTeacherRes>("api_addTeacher", {
    invalidate: [["rpc", "api_listTeachers"]],
    onSuccess: () => {
    ...
    onError: (err) => toast.error(err.message.replace(/\[.*\]$/, "") || "Could not add teacher."),
  });
                ...
                <Card className="h-full border-[#F7F2E8]/10 bg-[#17131D] transition-colors hover:border-[#D6A84F]/40">
                  <CardContent className="flex h-full flex-col pt-5">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-white/[0.04] pt-3 text-xs text-[#F7F2E8]/45">
                      ...
                      {t.profileIncomplete && (
                        <span className="text-amber-300">{t.missingFields?.length ?? 0} field(s) missing</span>
                      )}
                    </div>
                  </CardContent>
                </Card>
