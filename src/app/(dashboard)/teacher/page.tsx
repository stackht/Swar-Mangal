...
export default function TeacherDashboard() {
  const { classes, students, practice, assignments , currentTeacherId } = useAcademyData();
  const { user } = useAuth();
  const todayClasses = classes
    .filter((c) => c.teacher_id === currentTeacherId && new Date(c.start_time).toDateString() === new Date().toDateString())
    .sort((a, b) => +new Date(a.start_time) - +new Date(b.start_time));
  const myStudents = students.filter((s) => classes.some((c) => c.teacher_id === currentTeacherId && c.student_ids.includes(s.id)));
  ...
  const pendingReview = assignments.filter((a) => a.teacher_id === currentTeacherId && a.status === "submitted");
