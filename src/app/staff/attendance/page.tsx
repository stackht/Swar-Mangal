...
const selectClass =
  "h-11 rounded-2xl border border-[#F7F2E8]/12 bg-[#0B0A10] px-3 text-sm text-[#F7F2E8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D6A84F]/60";
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={listVariants} className="space-y-6">
      <motion.div variants={fadeUp}>
        ...
        <h1 className="mt-1 text-2xl font-semibold tracking-tight text-[#F7F2E8]">Attendance</h1>
      ...
      {roster.isError ? (
        <motion.div variants={fadeUp}>
          <p className="rounded-2xl border border-red-400/30 bg-red-400/10 p-6 text-sm text-red-300">
            ...
            <button type="button" onClick={() => roster.refetch()} className="font-medium text-[#D6A84F]">
              Retry
            </button>
          </p>
        </motion.div>
        ...
        <motion.div variants={fadeUp}>
          <div className="flex flex-col items-center rounded-2xl border border-dashed border-[#F7F2E8]/15 bg-white/[0.02] px-6 py-16 text-center">
            <UserRoundCheck className="mb-3 h-9 w-9 text-[#F7F2E8]/25" aria-hidden />
            <p className="text-sm font-medium text-[#F7F2E8]/75">No active students</p>
            <p className="mt-1 text-xs text-[#F7F2E8]/40">Nothing to mark for this filter.</p>
          </div>
        </motion.div>
      ) : (
              ...
              return (
                <li key={s.studentId} className="flex flex-col gap-3 px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-[#F7F2E8]">{s.name}</p>
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    ...
                    <button
                      ...
                      className={cn(
                        "flex h-9 w-9 items-center justify-center rounded-xl border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D6A84F]/60 disabled:opacity-50",
                        present
                          ? "border-emerald-400/50 bg-emerald-400/20 text-emerald-300"
                          : "border-[#F7F2E8]/12 text-[#F7F2E8]/60 hover:border-emerald-400/40 hover:text-emerald-300",
                      )}
                    >
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        absent
                          ...
                          : "border-[#F7F2E8]/12 text-[#F7F2E8]/60 hover:border-red-400/40 hover:text-red-300",
                      )}
                    >
